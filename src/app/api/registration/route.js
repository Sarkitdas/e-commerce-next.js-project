...
=======
  ...
  try {
    verificationToken = await CreateToken(body.email);
  } catch (tokenErr) {
    console.error("Token creation failed:", tokenErr);
    verificationToken = null; // optional: handle gracefully
  }
  ...
  try {
    if (verificationToken) {
      ...
      await transporter.sendMail({
        ...
        to: body.email,
        subject: "Verify Your Email",
        html: `<h3>Email Verification</h3><p>Please click the link below for verify:</p><a href="${verifyURL}" target="_blank">${verifyURL}</a>`,
      });
    }
  } catch (mailErr) {
...
>>>>>>> c6543551de25bc4dfe69c3166e18fdf335e15e24
}
